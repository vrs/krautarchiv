...
sub add_thread_data {
    ...
    my $sth = $self->{dbh}->prepare("INSERT OR IGNORE INTO 
                                    `threads`(`board_id`,`thread_id`,`content_length`) 
                                    VALUES (?,?,?)");
...
sub add_post {
    ...
    my $sth_1 = $self->{dbh}->prepare("INSERT OR IGNORE INTO 
                                      `posts`(`board_id`,`thread_id`,`post_id`,
...
sub add_file {
    ...
    my $sth_1 = $self->{dbh}->prepare("INSERT OR IGNORE INTO `files`(`path`,`timestamp`,`md5`)
                                       VALUES(?,?,?)");
...
sub add_rrd_data {
    ...
    my $sth = $self->{dbh}->prepare("INSERT OR IGNORE INTO `rrd`(`board_id`,`time`,`posts`)
                                     VALUES(?,?,?)");
    $sth->execute($board_id, $time, $value);
}

sub add_rrd_year_data {
    my $self = shift;
    my $board_id = shift;
    my $time = shift;
    my $value = shift;

    my $sth = $self->{dbh}->prepare("INSERT OR IGNORE INTO `rrd_year`(`board_id`,`time`,`posts`)
                                     VALUES(?,?,?)");
...
sub add_file_to_post {
    ...
    my $sth = $self->{dbh}->prepare("INSERT OR IGNORE INTO 
                                    `post_files`(`file_id`,`posts_rowid`,`filename`)
                                    VALUES(?,?,?)");
...
sub get_post {
    ...
    my $sth = $self->{dbh}->prepare("SELECT `posts_rowid`,`thread_id`,
                                            `subject`,`user`,`date`,`text`
                                     FROM `posts`
...
sub get_file_info_by_file_id {
    ...
    my $sth = $self->{dbh}->prepare("SELECT `board`,`board_id`,`thread_id`,
                                            `post_id`,`file_id`,`filename`
                                     FROM `post_files`
...
sub search_posts {
    my $self = shift;
    my $search = shift || croak("need search");
    
    my $sth = $self->{dbh}->prepare("SELECT `board`,`board_id`,`thread_id`,`post_id`,
                                            `subject`,`user`,`date`,`text`
                                     FROM `posts`
                                     JOIN `boards` USING(`board_id`)
                                     WHERE `text` LIKE ?
                                     ORDER BY `posts_rowid` DESC
                                     LIMIT 10");
    $sth->execute("%$search%");
    my @post_list = ();

    while(my ($board, $board_id, $thread_id, $post_id, $subject, $user, $date, $text) = $sth->fetchrow) {
        push(@post_list, {
                          board => $board,
                          board_id => $board_id,
                          thread_id => $thread_id,
...
sub get_file_list {
    ...
    my $sth = $self->{dbh}->prepare("SELECT `file_id`,`path`,`md5`,`board`,`board_id`,
                                            `thread_id`,`post_id`,`filename`
                                     FROM `files`
    ...
    while(my ($file_id, $path, $md5, $board, $board_id,
              $thread_id, $post_id, $filename) = $sth->fetchrow) {
...
sub get_post_time_by_board_id {
    ...
    my $sth = $self->{dbh}->prepare("SELECT strftime('%s',min(`date`)),
                                            strftime('%s',max(`date`))
                                     FROM `posts`
                                     WHERE `board_id` = ?");
    $sth->execute($board_id);
...
sub get_last_rrd_time_by_board_id {
    my $self = shift;
    my $board_id = shift || croak("gibe board_id");
    
    print "$board_id\n";
    my $sth = $self->{dbh}->prepare("SELECT max(`time`) FROM `rrd` WHERE `board_id` = ?");
    $sth->execute($board_id);
    ...
    if($time) {
        ...
        return get_first_post_time_by_board_id($self,$board_id);
    }
}

sub get_last_rrd_year_time_by_board_id {
    my $self = shift;
    my $board_id = shift || croak("gibe board_id");

    my $sth = $self->{dbh}->prepare("SELECT max(`time`) FROM `rrd_year` WHERE `board_id` = ?");
    $sth->execute($board_id);
...
sub get_total_threads {
    ...
    if($board_id) {
        $sth = $self->{dbh}->prepare("SELECT COUNT(DISTINCT `thread_id`)
                                      FROM `posts` WHERE `board_id` = ?");
        $sth->execute($board_id);
    } else {
    ...
    $dbh->do("CREATE TABLE
              IF NOT EXISTS `file_tags` (`tag_id` INTEGER NOT NULL,
                                         `file_id` INTEGER NOT NULL,
                                          ...
                                          ON DELETE CASCADE ON UPDATE CASCADE)");
    $dbh->do("CREATE TABLE
              IF NOT EXISTS `rrd` (`board_id` INTEGER,
                                   `time` INTEGER,
                                   `posts` INTEGER,
                                    PRIMARY KEY(`board_id`,`time`),
                                    FOREIGN KEY(`board_id`) REFERENCES `boards`(`board_id`)
                                    ON DELETE CASCADE ON UPDATE CASCADE)");

    $dbh->do("CREATE TABLE
              IF NOT EXISTS `rrd-year` (`board_id` INTEGER,
                                        `time` INTEGER,
                                        `posts` INTEGER,
                                         PRIMARY KEY(`board_id`,`time`),
                                         FOREIGN KEY(`board_id`) REFERENCES `boards`(`board_id`)
